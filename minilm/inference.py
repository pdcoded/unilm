...
def main(request_body):
    ...
    if request_body=={"context":"","answer":""}:
      return ("Nothing to extract questions from")
        ...
        model_recover_path = "/root/qg_model.bin"
        ...
        parser.add_argument("--input_file",type=str,default="text.pa.txt",help="Input file")
        ...
        parser.add_argument('--s2s_share_segment', action='store_false',
        ...
        parser.add_argument('--prod=true', type=str,
                            help="Dummy Argument")
        ...
        tokenizer = BertTokenizer.from_pretrained(
            bert_model, do_lower_case=args.do_lower_case)
        ...
        for model_recover_path in glob.glob(model_recover_path.strip()):
            ...
            model = BertForSeq2SeqDecoder.from_pretrained(bert_model, state_dict=model_recover, num_labels=cls_num_labels, num_rel=pair_num_relation, type_vocab_size=type_vocab_size, task_idx=3, mask_word_id=mask_word_id, search_beam_size=args.beam_size,
            ...
            #input_lines = [input_text] #[x.strip() for x in fin.readlines()]
            #input_lines = input_text.split(".")
            #input_lines = [x.strip() for x in input_lines if x!="" and len(x.split(" "))>2]
    ...
    return ({"question": output_lines[0], "answer":request_body["answer"]})
